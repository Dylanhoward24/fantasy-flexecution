...
router.get('/brett', (req, res) => {
    ...
    JOIN "users"
        ON "users"."id" = "players"."created_by_user_id"
    WHERE (("playersRankings"."user_id" = '14') AND ("positions"."position" = 'WR'))
    ...
    .catch((error) => {
        console.log('get brett wide-receivers error', error);
        res.sendStatus(500);
    });
});
...
router.get('/kyle', (req, res) => {
    ...
    JOIN "users"
        ON "users"."id" = "players"."created_by_user_id"
    WHERE (("playersRankings"."user_id" = '15') AND ("positions"."position" = 'WR'))
    GROUP BY "playerId", "firstName", "lastName", "team", "position", "tags", "tier", "tierRank", "hostId"
    ORDER BY "tier" ASC, "tierRank" ASC
    `;
    pool.query(sqlText)
    ...
    .catch((error) => {
        console.log('get kyle wide-receivers error', error);
        res.sendStatus(500);
    });
});
